...
export default function Landing() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-slate-950">
      ...
      {/* Gradient overlays */}
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-brand-900/40 via-slate-950 to-slate-950" />
      <div className="pointer-events-none absolute -left-32 top-1/4 h-96 w-96 rounded-full bg-brand-500/10 blur-3xl animate-pulse" />
      <div className="pointer-events-none absolute bottom-0 right-0 h-80 w-80 rounded-full bg-cyan-500/10 blur-3xl animate-pulse delay-1000" />

      {staticDemo && (
        <div className="relative z-10 border-b border-amber-500/30 bg-amber-950/40 px-6 py-2 text-center text-sm text-amber-200/90 backdrop-blur-sm">
          Statik demo: backend o‘chirilgan — ma’lumotlar faqat brauzerda (yangilansa qayta tiklanadi).
        </div>
      )}
      
      <header className="relative z-10 mx-auto flex max-w-6xl items-center justify-between px-6 py-8">
        <div className="font-display text-xl font-semibold tracking-tight text-white hover:text-brand-400 transition-colors duration-300">
          SOF QURILISH 
        </div>
        <Link
          to="/login"
          className="group relative overflow-hidden rounded-lg bg-brand-600 px-6 py-2.5 text-sm font-medium text-white shadow-lg shadow-brand-900/30 transition-all hover:bg-brand-500 hover:scale-105"
        >
      ...
      <main className="relative z-10 mx-auto max-w-6xl px-6 pb-24 pt-12 md:pt-20">
        <div className="hero-badge inline-block rounded-full bg-brand-500/10 px-4 py-1.5 text-sm font-medium text-brand-400 backdrop-blur-sm">
          ⚡ 3 darajali davlat nazorati
        </div>
        
        <h1 className="hero-title mt-6 max-w-3xl font-display text-4xl font-bold leading-tight text-white md:text-5xl lg:text-6xl">
          Qurilish korxonalari va nazorat organlari uchun
          <span className="text-brand-400"> yagona platforma</span>
        </h1>
        
        <p className="hero-description mt-6 max-w-2xl text-lg text-slate-400 leading-relaxed">
        ...
        <div className="cards-grid mt-14 grid gap-6 md:grid-cols-3">
          {[
            ...
            <div
              key={c.title}
              className="card-item group relative rounded-2xl border border-white/10 bg-gradient-to-br p-6 backdrop-blur-sm transition-all duration-300 hover:scale-105 hover:border-brand-500/30 hover:shadow-xl hover:shadow-brand-500/10"
            ...
            >
              ...
              <div className="mt-4 flex items-center text-xs text-brand-400 opacity-0 group-hover:opacity-100 transition-all">
